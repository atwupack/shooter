...
impl<T: Eq + Hash> Sounds<T> {
    ...
    pub fn load_sound(&mut self, sound: T, file: &str) -> AudioResult<()> {
    ...
    pub fn play_sound(&self, sound: &T) -> AudioResult<()> {
        let bytes = self.sound_store.get(sound).ok_or(SoundNotFound)?;
